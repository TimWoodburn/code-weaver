...
export function generateDOT(artifacts: Artifact[]): string {
  ...
  tierOrder.forEach((tier, index) => {
    ...
    tierArtifacts.forEach(artifact => {
      const nodeId = sanitizeNodeId(artifact.path);
  ...
  // Create a map for quick artifact lookup by path
  const artifactMap = new Map<string, Artifact>();
  artifacts.forEach(a => artifactMap.set(a.path, a));
  
  // Add edges for dependencies
  lines.push('  // Dependencies');
  artifacts.forEach(artifact => {
    const sourceId = sanitizeNodeId(artifact.path);
    
    artifact.dependencies.forEach(dep => {
      const targetArtifact = artifactMap.get(dep);
      if (targetArtifact) {
        ...
        lines.push(`  "${sourceId}" -> "${targetId}"${edgeStyle ? ` [${edgeStyle}]` : ''};`);
      }
    });
  });
